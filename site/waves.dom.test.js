...
function makeWorld(opts) {
  ...
  const sandbox = {
    ...
    getComputedStyle() {
      return {
        getPropertyValue(k) { return (opts.palette && opts.palette[k]) || ''; },
      };
    },
...

{
  ...
  world.sandbox.getComputedStyle = () => ({ getPropertyValue: () => '#ff0000' });
  world.observers.mutate.forEach((fn) => fn());
  check('the theme toggle repaints it', sea.palette.sea[0] === '#ff0000');
  sea.destroy();
}
